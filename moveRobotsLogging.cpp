...
std::string createLogMessage(const std::string& status, const sf::Vector2f& position) {
...
int main() {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        // Assign log file
        robot.logFile = "robot_logs/robot" + std::to_string(i + 1) + ".txt";
        ...
        logToFile(robot.logFile, createLogMessage("Initialized", robot.shape.getPosition()));
        ...
        for (auto& robot : robots) {
            ...
            if (robot.stuck) {
                ...
                if (timeSinceLastLog >= 3) {
                    logToFile(robot.logFile, createLogMessage("ERROR: Stuck", robot.shape.getPosition()));
            ...
            if (changedDirection) {
                logToFile(robot.logFile, createLogMessage("Changed direction", robot.shape.getPosition()));
            }

            // Log position every 3 seconds
            if (timeSinceLastLog >= 3) {
                logToFile(robot.logFile, createLogMessage("Moving", robot.shape.getPosition()));
